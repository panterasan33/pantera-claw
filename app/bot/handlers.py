...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result.message_type == MessageType.TASK:
        ...
        try:
            ...
            if memory_id:
                item_id = memory_id
                logger.info(f"Note persisted: id={memory_id}")
        except Exception as e:
        ...
        try:
            ...
            if memory_id:
                item_id = memory_id
                logger.info(f"Disclosure persisted: id={memory_id}")
        except Exception as e:
            logger.warning(f"Failed to persist disclosure: {e}")
    ...
    try:
        ...
        if result.message_type == MessageType.TASK:
            ...
            try:
                ...
                logger.warning(f"Failed to persist note from voice: {e}")
        elif result.message_type == MessageType.DISCLOSURE:
            data = result.extracted_data
            summary = data.get("summary")
            try:
                ...
                logger.warning(f"Failed to persist disclosure from voice: {e}")
    ...
    try:
        ...
        if result.message_type == MessageType.TASK:
            ...
            try:
                ...
                logger.warning(f"Failed to persist note from image: {e}")
        elif result.message_type == MessageType.DISCLOSURE:
            data = result.extracted_data
            summary = data.get("summary")
            try:
                ...
                logger.warning(f"Failed to persist disclosure from image: {e}")
