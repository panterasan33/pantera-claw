...
Pantera is a personal assistant with two entry points: **Telegram bot** and **web app**. Both share the same Postgres backend. Classification runs on text (LLM + rule-based fallback); voice and images are transcribed/extracted first, then classified. Every capture now lands in an inbox ledger and can emit interaction events for confirmations, corrections, and edits.
...
```
Telegram → webhook → update_queue → handlers → inbox ledger → orchestrator → classify → persist (Task/Reminder/Memory)
...
| **Orchestrator** | `app/services/orchestrator.py` | Inbox logging, routing, correction capture, episodic events |
...
| `InteractionEvent` | `interaction_events` | routing outcomes, confirmations, re-labels, edits |
...
| GET | `/api/brain/events` | Recent interaction and correction history |
| GET | `/api/search?q=` | Semantic search across items and correction history |
...
| Input | Handler | Flow |
|-------|---------|------|
| Text | `handle_message` | log inbox → classify → route → reply + correction keyboard |
| Voice | `handle_voice` | download → Whisper → log inbox → classify → route |
| Photo | `handle_photo` | download → Vision (gpt-4o-mini) → log inbox → classify → route |
| Document | `handle_document` | log inbox for manual/later processing |
| Callbacks | `handle_callback` | reminder actions, confirmations, reclassifications, edit requests |
...
- **Embed:** On create/update of Task, Reminder, MemoryItem, InboxItem, and InteractionEvent (via `embedding_service`)
- **Search:** `GET /api/search?q=` → embed query → pgvector cosine distance across tasks, reminders, memory, and interaction history
- **Question answering:** retrieve grounded context first, then synthesize a constrained answer when a model is available
